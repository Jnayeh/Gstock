...
// created_at: the time that the item was created. It will be automatically handled by SQLite
  ...
  static Future<List<Map<String, dynamic>>> getAllByID(int idEmprunt) async {
    ...
    return db.query(table,
        where: "idEmprunt = ?", whereArgs: [idEmprunt], orderBy: id + ' DESC');
  }
  ...
  static Future<List<Map<String, dynamic>>> getAllEmprunt() async {
    ...
    return await db.rawQuery(
        "SELECT ES.id,ES.qte, idComposant, idEmprunt, idMembre FROM $table ES, emprunts E  WHERE ES.idEmprunt = E.id ;");
  }
  ...
  static Future<int> update(int id, ComposantEmprunt emprunt) async {
    final db = await COMPOSANT_EMPRUNTHelper.db();
    ...
    final result = await db
        .update(table, emprunt.toMap(), where: "id = ?", whereArgs: [id]);
  ...
  static Future<void> delete(int id) async {
    ...
    try {
      ...
      Composant? composant =
          await COMPOSANTHelper.getItem(emprunt!['idComposant']!);
